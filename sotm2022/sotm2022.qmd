...
The missing data currently limit the use of OSM data in active travel planning, however this does mean that the use of OSM data should be dismissed. Following Nelson et al's (2021) argument regarding the need to make crowdsourced data more actionable, we decided to recategorize OSM data based on Inclusive Mobility guide (ref), a guide that outlines the best practices in creating inclusive transport. Indeed, it could be reasoned that by demonstrating the potential of OSM data in the policy-making contexts, citizens and policy makers would be incentivised to contribute to OSM data and encourage its use in bottom-up approach to planning.
