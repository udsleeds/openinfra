...
?t.test
anova1 = aov(leeds_traffic_osm_subset_clean$all_motor_vehicles ~ leeds_traffic_osm_subset_clean$highway)
summary(anova1)
...
cor.test(wy_traffic_osm_subset_recoded$all_motor_vehicles, wy_traffic_osm_subset_recoded$road_class,
         method = "spearman")
cor.test(wy_traffic_osm_subset_clean$all_motor_vehicles, as.numeric(as.factor(wy_traffic_osm_subset_clean$highway)),
         method = "spearman")

wy_traffic_osm_subset_clean %>% 
  mutate(highway_num = as.numeric(as.factor(wy_traffic_osm_subset_clean$highway))) %>% 
  select(highway, highway_num) %>% 
  sf::st_drop_geometry() %>% 
  table()
as.numeric(as.factor(wy_traffic_osm_subset_clean$highway))
wy_traffic_osm_subset_recoded$road_class
...
tmap_mode("view")
tm_shape(wy_traffic_2019_buffer)+
  tm_dots("all_motor_vehicles")+
  tm_shape(wtosr_na)+
  tm_lines()


wtosr_na = wy_traffic_osm_subset_recoded %>% filter(is.na(road_class))


```

```{r}
hi_interested_alt = c( "primary", "residential", "secondary", "tertiary",  "trunk", "unclassified")
wy_traffic_osm_subset_clean_alt = wy_traffic_osm_subset %>% filter(highway %in% hi_interested_alt)
boxplot_leeds_traffic_association = boxplot(wy_traffic_osm_subset_clean_alt$all_motor_vehicles ~ wy_traffic_osm_subset_clean_alt$highway)

wy_traffic_osm_subset %>% filter(highway == "service") %>% mapview()
```
