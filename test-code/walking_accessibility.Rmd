...
st_geometry(leeds_pct) %>% plot(reset = FALSE)
wy_walking_fr %>% select(pedestrian_friendly) %>% filter(!is.na(pedestrian_friendly)) %>% plot(add = TRUE)
```
...
```{r}
sf_col_table(wy_walking_fr, "pedestrian_friendly")
sf_col_table(man_walking_fr, "pedestrian_friendly")
```
