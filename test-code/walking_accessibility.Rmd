...
# examples:
# proportion_sf(wy_walking, x = 3:5)
# proportion_sf (wy_walking,
#               x = c("highway",
#                     "waterway",
#                     "aerialway"))
```
...
## plotting pedestrian network in WY

```{r}
wy_walking_fr <- wy_walking %>% 
  mutate(pedestrian_friendly = case_when(
    highway %in% highway_pedestrian_yes ~ "yes",
    highway %in% highway_pedestrian_no ~ "no",
...
st_geometry(leeds_pct) %>% plot(reset = FALSE)
wy_walking_fr[leeds_pct, op = st_within] %>% select(pedestrian_friendly) %>% filter(pedestrian_friendly == "yes") %>% plot(add = TRUE)
```
...
```{r}
# recategorizing GM dataframe
man_walking_fr <- man_walking %>% 
  mutate(pedestrian_friendly = case_when(
    highway %in% highway_pedestrian_yes ~ "yes",
    highway %in% highway_pedestrian_no ~ "no",
...
```{r}
# preparing data for plotting by turning it from wide to long df

man_ped_n_perc <- man_walking_fr %>% 
  st_drop_geometry() %>% 
  group_by(pedestrian_friendly) %>% 
  summarise(n_man = n(),
            percent_man = ((n_man / nrow(man_walking_fr) ) * 100) %>% signif(2)
            )

wy_ped_n_perc <- wy_walking_fr %>% 
  st_drop_geometry() %>% 
  group_by(pedestrian_friendly) %>% 
  summarise(n_wy = n(),
            percent_wy = ((n_wy / nrow(wy_walking_fr) ) * 100) %>% signif(2)
            )

joined_man_wy_ped_n_perc <- left_join(man_ped_n_perc,
                                 wy_ped_n_perc)

joined_man_wy_ped_perc_long <- joined_man_wy_ped_n_perc %>% 
  pivot_longer(
    col = c("percent_man",
            "percent_wy"),
    values_to = "percent"
...
joined_man_wy_ped_n_long <- joined_man_wy_ped_n_perc %>%
  pivot_longer(
    col = c("n_man",
            "n_wy"),
    values_to = "n"
...
joined_man_wy_ped_long <- left_join(joined_man_wy_ped_perc_long,
                                    joined_man_wy_ped_n_long,
                                    by = "pedestrian_friendly")


# plotting 
joined_man_wy_ped_long %>%   
  ggplot(aes(
    x = pedestrian_friendly,
    y = percent,
    fill = name.x
  )) +
...
```{r}
wy_pedestrian <- recode_pedestrian(wy_walking)
wy_pedestrian_leeds <- wy_pedestrian[leeds_pct, op = st_within]
wy_pedestrian_leeds_yes <- wy_pedestrian_leeds %>% filter(pedestrian_friendly == "yes")

wy_pedestrian_leeds %>%  select(osm_id, name, highway, pedestrian_friendly, foot, footway, sidewalk) %>% filter(!is.na(pedestrian_friendly)) %>% mapview(zcol = "pedestrian_friendly", burst = TRUE)
```

```{r}

wy_pedestrian_leeds %>% filter(pedestrian_friendly == "yes" | pedestrian_friendly == "maybe") %>% select(osm_id, name, highway, pedestrian_friendly, foot, footway, sidewalk )%>% mapview(zcol = "pedestrian_friendly", burst = TRUE)
```

```{r}

# wy_pedestrian_leeds %>% filter(pedestrian_friendly == "maybe") %>% mapview()
```


```{r}
# wy_pedestrian_leeds %>% mutate(ped_yes = case_when(pedestrian_friendly == "yes" ~"yes")) %>% 
#                                  mutate(ped_maybe = case_when(pedestrian_friendly == "maybe" ~"maybe")) %>%
#  select(osm_id, name, highway, pedestrian_friendly, foot, footway, sidewalk, ped_yes, ped_maybe) %>%  filter(!is.na(ped_yes) | !is.na(ped_maybe)) %>%  mapview(zcol = c("ped_yes", "ped_maybe"), legend = TRUE, burst = TRUE)
```
...
```{r}
wy_pedestrian_leeds %>% filter(osm_id == 257801955 | osm_id == 	1709451) %>% mapview()

# osmextract::oe_get_keys(zone = "West Yorkshire")
# 
# wy_walking_testing <- osmextract::oe_get("West Yorkshire",
#                                  force_vectortranslate = TRUE,
#                                  extra_tags = "informal"
#                                  )

# wy_walking_testing %>% filter(!is.na(informal)) 
# sf_col_table(wy_walking, x = "sidewalk") %>% sum() 
# sf_col_table(wy_walking_testing, x = "informal") %>% sum() 
