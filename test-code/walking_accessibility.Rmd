...
wy_walking_nogeom <- wy_walking_nogeom %>% 
...
wy_walking_nogeom %>% filter(pedestrian == "yes")
## just a quick sanity check that numbers match
wy_walking_nogeom %>% filter(pedestrian == "yes" | pedestrian == "maybe" | pedestrian == "no") %>% nrow() # 181302 rows that had been recoded
wy_walking_nogeom %>% filter(is.na(highway)) %>% nrow() # 53311 rows that have NAs in key:highway =*, hence no recoding
(181302 + 53311) == (wy_walking_nogeom %>% nrow()) # value must be TRUE

```


### categorization of key:foot
