...
It has to be noted that this function is a simplification of all the requirements outlined in guide. There are a couple of reasons for this. First, while there is a large number of tags in OSM to create detailed maps, but the values imputed are not always IM guide-friendly. For example, in the guide 6 different types of tactile paving are outlines, yet those types are not key values in the OSM as it seems to only be focused on the presence/absence of tactile paving. This, then, leads not only to the idea that not all OSM data can be sensibly recategorized based on the IM guide but also that it is highly unlikely that a single function could capture the detailed specifications outlined in the IM guide. 
...
```{r}
inclusive_mobility_get_test = function(osm_sf) {
  im_footway_tags = c("footway","sidewalk","cycleway","foot","segregated")
  
  osm_sf_im = if("kerb" %in% colnames(osm_sf)){
    osm_sf %>%
      dplyr::mutate(im_kerb = dplyr::if_else(kerb == "flush", "flush", "other"))
  } else{
    warning("'kerb' column is missing")
  } 
  # footway is a ‘pavement’ adjacent to a road
  if(all(im_footway_tags %in% colnames(osm_sf))){
    osm_sf %>%
        dplyr::mutate(im_footway = dplyr::case_when(
        footway %in% c("left", "right", "both", "sidewalk") |
        sidewalk %in% c("left", "right", "both", "yes", "separate") |
    ...
    warning("A column is missing, hence recategorization cannot be completed.")
  }
...
```{r}
if(all(im_footway_tags %in% colnames(wy_joined))){
    wy_joined %>%
        dplyr::mutate(im_footway = dplyr::case_when(
        footway %in% c("left", "right", "both", "sidewalk") |
        sidewalk %in% c("left", "right", "both", "yes", "separate") |
    ...
    warning("A column is missing, hence recategorization cannot be done.")
  } %>% pull(im_footway) %>% table()
```
