...
inclusive_mobility_get_test = function(osm_sf) {
  im_footway_tag = c("footway","sidewalk","cycleway","foot","segregated")
  im_kerb_tag = "kerb"
  im_footpath_tag = c("highway", "foot", "access", "segregated")
  
  osm_sf = if(im_kerb_tag %in% colnames(osm_sf)){
    osm_sf %>%
      dplyr::mutate(im_kerb = dplyr::if_else(kerb == "flush", "flush", "other"))
  } else{
  ...
  # footway is a ‘pavement’ adjacent to a road
  osm_sf = if(all(im_footway_tag %in% colnames(osm_sf))){
    osm_sf %>%
        dplyr::mutate(im_footway = dplyr::case_when(
        footway %in% c("left", "right", "both", "sidewalk") |
        sidewalk %in% c("left", "right", "both", "yes", "separate") |
    ...
    warning("A column is missing, hence recategorization cannot be completed. See documentation for details.")
  }
   # footpath is any other right of way for pedestrians, that does not run adjacent to a road.
  osm_sf = if(all(im_footpath_tag %in% colnames(osm_sf))){
    osm_sf %>% 
      dplyr::mutate(im_footpath = dplyr::case_when(
        highway %in% "footway" & 
      im_footway %in% "no" | 
...
```{r}
wy_im_test = inclusive_mobility_get_test(wy_joined)
wy_im_test %>% pull(im_kerb) %>% table()
wy_im_test %>% pull(im_footway) %>% table()

wy_im_test1 = inclusive_mobility_get_test(wy_sample)
wy_im_test1 %>% pull(im_kerb) %>% table()
wy_im_test %>% pull(im_footway) %>% table()
```
