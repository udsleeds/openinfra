...
In this section we focus on two python packages commonly utilised to obtain and assess **Open Street Map** ([OSM](https://www.openstreetmap.org/)) data, namely [OSMnx](https://osmnx.readthedocs.io/en/stable/#) and [Pyrosm](https://pyrosm.readthedocs.io/en/latest/#), to complete necessary processing steps to work with OSM data for transport research comprising:
...
Specifically, we will recreate the cycling network for Leeds.
From function documentation we know:
...
We will now take a look at the `get _data_by_custom_criteria` function documentation
...
Comparing to the default cycling network (`get_network(network_type='cycling')`)
```
