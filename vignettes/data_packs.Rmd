...
<br><br>
The transport infrastructure data packs contain OSM infrastructure data for a given Local Authority District (LAD) as defined by the polygons of this map of [UK LADs](https://github.com/udsleeds/openinfra/blob/main/data-small/lads_joined_2021.geojson). 
<br><br>
Whilst these LADs are currently being used to define the spatial area covered by each infrastructure data pack, any new set of updated boundaries (as a geojson file) can also be used to create the infrastructure data packs. 
<br><br>

### Data Packs Additional Data - The Functions
...
A current example data pack for "Leeds" - contains outputs: <br>
...
- Inclusive Mobility (unfortunately not mapped here due to map filesize) 
<br><br>


### Default OSM Highways

Please note that a 7.5Km circular buffer (centred at Leeds City Centre) has been applied to this map so that the map can be hosted on this website. (max filesize 25Mb) 
