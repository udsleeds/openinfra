...
## Getting data with `osmdata`

Another package for getting OSM data is `osmdata`, which gets data from the Overpass API rather than from bulk .pbf files.
`osmdata` is ideal for getting small amounts of data from OSM, e.g. the boundary representing Leeds, as shown in the code chunk below.
