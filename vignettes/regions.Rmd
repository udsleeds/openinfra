...
```
Due to a [change](https://github.com/r-spatial/sf/issues/1649#issuecomment-860231045) in the spherical coordinates (S2) within the sf package that is known to cause issues ([1](https://github.com/r-tmap/tmap/issues/577), [2](https://github.com/r-tmap/tmap/issues/571)) when plotting geospatial data we make use of a known [fix](https://github.com/r-spatial/sf/issues/1649#issuecomment-860231045) to use the old coordinate system by setting `sf::sf_use_s2(use_s2 = FALSE)`
```{r}
...
tmap_options(max.categories = 78) # We increase max categories so that every transport region has their own colour
...
```{r, ig.height=15.25, fig.width=9}
# Here we create the interactive plot of Englands transport regions
transport_regions %>% tmap::qtm(fill = 'Region_name')
```
