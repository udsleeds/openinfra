...
Before you move on to plotting, download an OSM dataset representing central Leeds. A smaller dataset requires less computational power, hence reducing the plotting time. If you want to plot all Leeds, go ahead but you will have to adjust the code accordingly.
```{r}
url = "https://github.com/udsleeds/openinfra/releases/download/v0.1/leeds_central.RDS"
download.file(url, 
              destfile = "leeds_central.RDS") # downloading the file to the working directory; if you do not know where your working directory is, type `getwd()` to your console.

leeds_central = readRDS("leeds_central.RDS") # reading the file
```
...
# import and save as an object (uncomment first); 
leeds_tn = readRDS("leeds_tn.RDS")
```
...
leeds_tn %>% select(foot) %>% plot() # plot
dev.off() # closes the file
# the image should have been saved in your working directory
```
...
[`tmap`](https://r-tmap.github.io/tmap/) is a flexible package that allows to build both static and interactive maps in R. In this section I will show you how to use its one-liner `qtm()` function to build interactive maps (note: it might be code-efficient, but not always time-efficient! It might take time to build maps, especially the large ones).
